...
p = subprocess.Popen("{{ mysql_path }} -u {{ nagios_mysql_user|default('nagios') }} -p{{ mysql_password }} -e 'show slave status' -E", stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=True)
